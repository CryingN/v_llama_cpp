...
fn C.llama_load_model_from_file(
	path &u8,
	params C.llama_model_params
) &C.llama_model
fn C.llama_model_default_params() C.llama_model_params
fn C.llama_model_get_vocab(
	model &C.llama_model
) &C.llama_vocab
fn C.llama_get_model(
	ctx &C.llama_context
) &C.llama_model

fn C.llama_context_default_params() C.llama_context_params
fn C.llama_init_from_model(
	model &C.llama_model,
	ctx_params &C.llama_context_params
) &C.llama_context

fn C.llama_batch_get_one(
	tokens		&i32,
	n_tokens	int
) C.llama_batch
fn C.llama_decode(
...
fn C.llama_vocab_n_tokens(
	vocab		C.llama_vocab
) int
fn C.llama_get_logits_ith(
	context		C.llama_context,
	last_pos	int
) &f32
fn C.llama_vocab_is_eog(
	vocab &C.llama_vocab,
	token int
) bool
fn C.llama_token_to_piece(
	vocab &C.llama_vocab,
	token int,
	buf &char,
	length int,
